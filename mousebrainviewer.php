...
<style>	
	...
	<button type="button" onClick="resett()">Reset</button>
...
<br>
<script type="text/javascript"  id="canvas">
function plus(){ // W key
	<?php for($i = 1; $i <= 8; $i++){ ?>
...
function resett() {
	<?php for($i = 1; $i <= 8; $i++){ ?>
		camera<?php echo$i; ?>.lookAt( (0,0,0) );
...
function S() {
	...
	<?php }	?>
		...
		var originalposition = new THREE.Object3D();
		...
		function init(){
			<?php for($i = 1; $i <= 8; $i++){ ?>
				...
				scene<?php echo$i; ?>.add( originalposition );
				originalposition.add(camera<?php echo$i; ?> );
				camera_pivot.add( camera<?php echo$i; ?> );	console.log(camera_pivot);		
			...
			<?php for($i = 1; $i <= 8; $i++){ ?>
				...
				renderer<?php echo$i; ?>.setSize(450, 400);				
		...
		<?php for($i = 1; $i <= 8; $i++){ ?>	
			...
			function onResourcesLoaded<?php echo$i; ?>(){	
				...
				//transparent=true to enable opacity
		...
		//this function and the following function are intended to add a hover effect to the mouse brain viewer
		//but do not work yet because the x and y coordinates are not calculated correctly
